...
export type CardTheme = "indigo" | "midnight" | "emerald" | "sunset" | "cyber" | "minimal";
...
const FORMATS: { id: CardFormat; name: string; desc: string; icon: React.ComponentType<{ className?: string }> }[] = [
  { id: "story", name: "Instagram Story (9:16)", desc: "Story, TikTok & Snapchat", icon: Smartphone },
  { id: "square", name: "Square Post (1:1)", desc: "Instagram & Feeds", icon: Square },
...
export default function GradeSocialCardStudio({ data }: GradeSocialCardStudioProps) {
  ...
  const [customNote, setCustomNote] = useState<string>("Locked in 🔒 • GradeCalculator.dev");
  ...
  const drawStoryFormat = (ctx: CanvasRenderingContext2D, width: number, height: number, isLight: boolean) => {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(width / 2 - 210, cardMargin + 70, 420, 62, 31);
    ctx.fillStyle = isLight ? "#eef2ff" : "rgba(99, 102, 241, 0.22)";
    ctx.fill();
    ...
    const metrics = [
      {
        ...
        val: data.statusText || (data.gpaPoint && data.gpaPoint >= 3.5 ? "Dean's List / Honors Tier" : "Passing with Distinction"),
      },
      {
        title: "4.0 GPA CONVERSION",
        val: showGPA && data.gpaPoint !== undefined ? `${data.gpaPoint.toFixed(2)} Quality Points` : "Standard Scale",
      },
      {
    ...
    ctx.fillStyle = isLight ? "#64748b" : "#94a3b8";
    ctx.fillText("Free • Fast • 100% Private Student Calculator", width / 2, footerY + 90);
    ctx.restore();
  };
  ...
  return (
    <div className="mt-8 pt-8 border-t border-slate-200 bg-gradient-to-b from-slate-50 via-white to-indigo-50/20 rounded-3xl p-5 sm:p-8 border border-slate-200/90 shadow-lg shadow-indigo-50 space-y-6">
      {/* Studio Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-100/90 text-indigo-800 text-xs font-bold uppercase tracking-wider mb-2">
            <Sparkles className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
            <span>Interactive Social Card Studio</span>
          </div>
          <h3 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">
            Design & Share Your Grade Achievement
          </h3>
          <p className="text-xs sm:text-sm text-slate-600 mt-1">
            Customize your aesthetic card, choose Story, Square, or Wide formats, and share with your friends or study groups.
          </p>
        </div>

        {/* Quick Share Buttons on Top Right */}
      ...
      <div>
        <label className="flex items-center gap-1.5 text-xs font-bold text-slate-700 uppercase tracking-wider mb-2.5">
          ...
          <span>1. Choose Card Aspect Ratio & Layout</span>
        </label>
      ...
      {/* Main Studio Interactive Grid: Left Controls | Right Live Canvas Preview */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        {/* Left Column: Interactive Design Customization Controls */}
        <div className="lg:col-span-6 space-y-5 order-2 lg:order-1">
              ...
              {Object.values(THEMES).map((t) => (
                <button
                  ...
                  className={`p-2.5 rounded-xl text-xs font-bold text-left border flex items-center gap-2 transition-all ${
                    selectedTheme === t.id
                      ? "border-indigo-600 bg-indigo-50/90 text-indigo-900 shadow-sm ring-2 ring-indigo-500/20"
          ...
          {/* Text Customization: Course Name */}
          <div className="space-y-3">
            <label className="flex items-center gap-1.5 text-xs font-bold text-slate-700 uppercase tracking-wider">
              <Type className="w-3.5 h-3.5 text-indigo-600" />
              <span>3. Course Title & Student Motto</span>
            </label>
            ...
            <div>
              <span className="text-[11px] font-semibold text-slate-500 block mb-1">Custom Caption / Status Note</span>
              <input
                type="text"
                value={customNote}
              ...
              <div className="flex flex-wrap gap-1.5">
                {MOTTO_PRESETS.map((motto) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 text-slate-700 border border-slate-200 transition-colors"
                  >
                    {motto}
                  </button>
                ))}
          ...
          <div>
            <label className="flex items-center gap-1.5 text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              ...
              <span>4. Privacy & Display Elements</span>
            </label>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
            <button
              ...
              className="w-full sm:flex-1 py-3 px-5 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white font-bold text-sm rounded-xl flex items-center justify-center gap-2 transition-all shadow-md shadow-indigo-200 min-h-[46px]"
            >
        ...
        {/* Right Column: Live Interactive Card Canvas Preview */}
        <div className="lg:col-span-6 space-y-2 order-1 lg:order-2">
          ...
          {/* Canvas Wrapper */}
          <div className="w-full overflow-hidden rounded-2xl border border-slate-300 shadow-xl bg-slate-950 flex items-center justify-center p-3 sm:p-5">
            <canvas
              ref={canvasRef}
              className={`w-auto object-contain transition-all duration-300 rounded-xl shadow-lg ${
                selectedFormat === "story"
                  ? "max-h-[420px] sm:max-h-[480px] aspect-[9/16]"
                  : selectedFormat === "square"
                  ? "max-h-[340px] sm:max-h-[380px] aspect-square"
          ...
          <div className="text-center text-[11px] text-slate-500 font-medium">
            💡 Tap <strong>Download</strong> or <strong>Copy Image</strong> to post on your Instagram Story, Snapchat, TikTok, or Discord!
          </div>
        </div>
      </div>
    </div>
